        ...
        if self.multi_label:
            ...
            try:
                ...
                classification_metrics = compute_classification_metrics(
                    self.video_preds, self.video_labels, num_classes
                )
                
                # Add to stats
                self.stats["accuracy"] = "{:.{prec}f}".format(
                    classification_metrics['accuracy'] * 100.0, prec=2
                )
                self.stats["macro_precision"] = "{:.{prec}f}".format(
                    classification_metrics['macro_precision'] * 100.0, prec=2
                )
                self.stats["macro_recall"] = "{:.{prec}f}".format(
                    classification_metrics['macro_recall'] * 100.0, prec=2
                )
                self.stats["macro_f1"] = "{:.{prec}f}".format(
                    classification_metrics['macro_f1'] * 100.0, prec=2
                )
                if classification_metrics['roc_auc'] is not None:
                    self.stats["roc_auc"] = "{:.{prec}f}".format(
                        classification_metrics['roc_auc'], prec=4
                    )
                
                # Log detailed metrics
                class_names = ['NonFight', 'Fight'] if num_classes == 2 else None
                log_classification_metrics(classification_metrics, class_names, logger)
...
class ValMeter(object):
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if self._cfg.DATA.MULTI_LABEL:
            ...
            # Compute comprehensive classification metrics on validation set
            try:
                all_preds = torch.cat(self.all_preds)
                all_labels = torch.cat(self.all_labels)
                num_classes = all_preds.size(1) if all_preds.dim() == 2 else 2
                
                classification_metrics = compute_classification_metrics(
                    all_preds, all_labels, num_classes
                )
                
                # Add key metrics to stats
                stats["val_accuracy"] = classification_metrics['accuracy'] * 100.0
                stats["val_macro_precision"] = classification_metrics['macro_precision'] * 100.0
                stats["val_macro_recall"] = classification_metrics['macro_recall'] * 100.0
                stats["val_macro_f1"] = classification_metrics['macro_f1'] * 100.0
                if classification_metrics['roc_auc'] is not None:
                    stats["val_roc_auc"] = classification_metrics['roc_auc']
                
                # Log detailed metrics for best epoch
                if flag:
                    class_names = ['NonFight', 'Fight'] if num_classes == 2 else None
                    logger.info("\n" + "="*60)
                    logger.info(f"BEST VALIDATION EPOCH {cur_epoch + 1}")
                    log_classification_metrics(classification_metrics, class_names, logger)
                    
            except Exception as e:
                logger.warning(f"Could not compute validation classification metrics: {e}")
...
def compute_classification_metrics(preds, labels, num_classes=2):
    """
    Compute comprehensive classification metrics for binary/multi-class classification.
    Args:
        preds (tensor): predictions with shape (N, num_classes) or (N,)
        labels (tensor): ground truth labels with shape (N,)
        num_classes (int): number of classes (default 2 for Fight/NonFight)
    Returns:
        metrics_dict (dict): dictionary containing:
            ...
            - roc_auc: ROC-AUC score (for binary or one-vs-rest)
    """
    # Convert to numpy
    if torch.is_tensor(preds):
        if preds.dim() == 2:  # (N, num_classes)
    ...
    if probs is not None:
        try:
            if num_classes == 2:
                ...
                roc_auc = roc_auc_score(true_labels, probs, multi_class='ovr', average='macro')
...
def log_classification_metrics(metrics_dict, class_names=None, logger=None):
    """
    Log classification metrics in a readable format.
    Args:
        metrics_dict (dict): output from compute_classification_metrics
        class_names (list): list of class names (e.g., ['NonFight', 'Fight'])
    ...
    if class_names is None:
        class_names = [f"Class_{i}" for i in range(len(metrics_dict['precision']))]
    ...
    # Overall metrics
    logger.info(f"Overall Accuracy: {metrics_dict['accuracy']*100:.2f}%")
    ...
    for i, class_name in enumerate(class_names):
        ...
        logger.info(f"  Precision: {metrics_dict['precision'][i]*100:.2f}%")
        logger.info(f"  Recall:    {metrics_dict['recall'][i]*100:.2f}%")
        logger.info(f"  F1-Score:  {metrics_dict['f1_score'][i]*100:.2f}%")
        logger.info(f"  Support:   {metrics_dict['support'][i]}")
    
    # Macro-averaged metrics
    logger.info("\nMacro-Averaged Metrics:")
    logger.info("-"*60)
    logger.info(f"Precision: {metrics_dict['macro_precision']*100:.2f}%")
    logger.info(f"Recall:    {metrics_dict['macro_recall']*100:.2f}%")
    logger.info(f"F1-Score:  {metrics_dict['macro_f1']*100:.2f}%")
